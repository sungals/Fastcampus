...
//  LMessenger
//
//  Created by SEONG MIN HAN on 12/28/23.
//
...
//        Empty().eraseToAnyPublisher()
        Just(object)
            ...
            .flatMap { value in
                Future { [weak self] promise in // Users/userId/ ..
    ...
    func loadUsers() -> AnyPublisher<[UserObject], DBError> {
        Future<Any?, DBError> { [weak self] promise in
            self?.db.child(DBKey.Users).getData(completion: { error, snapshot in
                if let error {
                    promise(.failure(DBError.error(error)))
                } else if snapshot?.value is NSNull {
                    promise(.success(nil))
        ...
        .flatMap { value in
            if let dic = value as? [String: [String: Any]] {
                ...
                return Fail(error: .invalidateType).eraseToAnyPublisher()
            }
        }
        .eraseToAnyPublisher()
    }
